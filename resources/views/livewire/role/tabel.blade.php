<div class="card-body">
    <div class="table-responsive text-nowrap" id="myTabel">
        ...
        <table class="table table-striped" id="categoriesTable">
            <thead>
                <tr>
                    <th>@lang('dash/index.name')</th>
                    <th>@lang('dash/index.actions')</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ router('roles.destroy', ['role' => $role->id]) }}" method="POST" style="display: flex; align-items: center; gap: 0.5rem;">
                            <a class="btn btn-info" href="{{ router('roles.show', ['role' => $role->id]) }}">@lang('dash/index.show')</a>
                            @can('role-edit')
                                <a class="btn btn-primary" href="{{ router('roles.edit', ['role' => $role->id]) }}">@lang('dash/index.edit')</a>
                            @endcan
                            @csrf
                            @method('DELETE')
                            @can('role-delete')
                                <button type="submit" class="btn btn-danger">@lang('dash/index.delete')</button>
                            @endcan
                        </form>
                    </td>
                </tr>
